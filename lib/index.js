		...
		if (live !== void 0) {
			for (const session of live.list()) {
				...
				try {
					...
					// This harness generation exposes the event log through
					// `snapshotEvents()` (seq-based); older builds carried a
					// plain `events` array.
					const events =
					...
					ctx.logger.warn(`usage-calendar: folding live session "${session.id}" failed: ${String(error)}`);
				}
			}
		}
